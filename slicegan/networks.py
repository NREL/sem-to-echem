...
def slicegan_nets(pth, Training, imtype, dk, ds, df, dp, gk, gs, gf, gp):
    ...
    class Generator(nn.Module):
        ...
        def forward(self, x):
            ...
            elif imtype in ['graphite']:
                out = torch.tanh(self.convs[-1](x))
            else:
...
def slicegan_rc_nets(pth, Training, imtype, dk, ds, df, dp, gk, gs, gf, gp):
    ...
    class Generator(nn.Module):
        def __init__(self):
            ...
            self.convs = nn.ModuleList()
            self.bns = nn.ModuleList()
            self.rcconv = nn.Conv3d(gf[-2], gf[-1], 3, 1, 'same')
            for lay, (k, s, p) in enumerate(zip(gk, gs, gp)):
                #self.convs.append(nn.Conv3d(gf[lay], gf[lay+1], k, s, p, bias=False))
                self.convs.append(nn.ConvTranspose3d(gf[lay], gf[lay+1], k, s, p, bias=False))
                self.bns.append(nn.BatchNorm3d(gf[lay+1]))
        ...
        def forward(self, x):
            ...
            #size = (64, 64, 64)
            size = (128, 128, 128)
            up = nn.Upsample(size=size, mode='trilinear', align_corners=True)
