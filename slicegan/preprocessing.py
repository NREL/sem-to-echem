...
def batch(data, type, l, sf):
    ...
    if type in ['png', 'jpg', 'tif2D']:
        ...
        for img in data:
            ...
            N_samples = 10000#int(img.shape[0]/l)*int(img.shape[1]/l)
            ...
            data = np.empty([N_samples, len(phases), l, l])
        ...
        img_min -= 0.25*img_rng
        img_max += 0.25*img_rng

        img = (img - img_min)/(img_max - img_min)
        
        ## Create a data store and add random samples from the full image
        N, x_max, y_max, z_max = img.shape[:]
        N_per = 16
        print('training image shape: ', img.shape)
        ...
        for dim in range(3):
            ...
            for i in range(N):
                ...
                if dim==0:
                    cnt = int(x_max / N_per)
                    data_i = img_i[::cnt, y:y+l, z:z+l]
                elif dim==1:
                    cnt = int(y_max / N_per)
                    data_i = np.transpose(img_i[x:x+l, ::cnt, z:z+l], (1, 0, 2))
                else:
                    cnt = int(z_max / N_per)
                    data_i = np.transpose(img_i[x:x+l, y:y+l, ::cnt], (2, 0, 1))
